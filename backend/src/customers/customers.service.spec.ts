...
describe('CustomersService', () => {
		...
		it('deve lançar NotFoundException se cliente não existir', async () => {
			...
			await expect(service.findOne('inexistent-id')).rejects.toThrow(
				new NotFoundException(
					'Cliente com ID inexistent-id não encontrado',
				),
			);
		});
	});
	...
	describe('update', () => {
		...
		it('deve atualizar um cliente com sucesso', async () => {
			...
			expect(fountainRepository.delete).toHaveBeenCalledWith({
		...
		it('deve lançar NotFoundException se cliente não existir', async () => {
			...
			await expect(
				service.update('inexistent-id', mockUpdateDto),
			).rejects.toThrow(
				new NotFoundException(
					'Cliente com ID inexistent-id não encontrado',
				),
		...
		it('deve lançar erro se CPF já estiver em uso por outro cliente', async () => {
			...
			await expect(
				...
				new BadRequestException('Já existe um cliente com este CPF'),
			);
		});
